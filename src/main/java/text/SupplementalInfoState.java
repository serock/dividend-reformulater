...
class SupplementalInfoState implements State {
    ...
    private static final Pattern patternStartsWithStateAndPercentage = Pattern.compile("^[A-Z][A-Za-z. ]{2,16}[a-z]\\s[0-9]{1,3}\\.[0-9]{2}");
    ...
    public void accept(final Context context, final String text) {
        ...
        if (matcher.find()) {
            ...
            if (matcher.find()) {
                ...
                matcher = patternPercent.matcher(text);
                percent = matcher.find(end) ? cleanPercent(matcher.group()) : "";
        ...
        matcher = patternStartsWithStateAndPercentage.matcher(text);
        if (matcher.find()) {
            final String[] lastRow = context.getLastSupplementalInfoRow();
